...
export default function Input(props) {
  const [inputDol, setInputDol] = React.useState(0);
  const [inputPpl, setInputPpl] = React.useState(0);
  ...
  const handleInputDol = (event) => {
    setInputDol(event.target.value);
  };

  const handleInputPpl = (event) => {
    setInputPpl(event.target.value);
  };
  ...
  useEffect(() => {
    props.inputAmount(((inputDol / 100) * percentage) / inputPpl, inputPpl);
  }, [percentage, inputDol, inputPpl, props]);
  ...
  return (
    <div className="input">
      ...
      <div className="input-bill  number-of-people">
